...
public class UserHibDao {
		...
		public static User getuser(int userid){
			...
			//Transaction tx = null;
			try{
				org.hibernate.Query query = session.createQuery("from User u where u.userId=?");
				((org.hibernate.Query) query).setLong(0, userid);
				@SuppressWarnings("unchecked")
				List<User> userlist = ((org.hibernate.Query) query).list();
		...
		public static List<User> select(){
			...
			Session session = HibernateUtil.getSession();
			try{
				org.hibernate.Query query = session.createQuery("from User u");
		...
		public static String findUserbyWeibo(String weiboid){
			Session session = HibernateUtil.getSession();
			try{
				org.hibernate.Query query = session.createQuery("from Weiboinfo u where u.weiboId=?");
				((org.hibernate.Query) query).setString(0, weiboid);
				@SuppressWarnings("unchecked")
				List<Weiboinfo>weiboinfoList = ((org.hibernate.Query) query).list();
				if(weiboinfoList == null || 0 == weiboinfoList.size()){
					return "nonuser";//用户名不存在
				}
				Iterator<Weiboinfo> it = weiboinfoList.iterator();
				Weiboinfo weiboinfo = it.next();
